...
- 완료: HALT/DISARM/EMERGENCY, ARM 재개, `backoffice_command` 명령 원장,
  ...
  - 승인 한도 표시 — 1거래 1%, 레버리지 (당시 7, **2026-09-04에 50으로 상향** —
    2026-09-03 계획서 §33.25), 세션 거래 8,
...
## 체결 스트림 클라이언트 (2026-09-01)

`ingest_agg_trade`를 부르는 것이 없었다. `BinanceUsdmTradeStream`이 그 자리다 —
연결을 잡고, 프레임을 넘기고, 끊기면 다시 붙는다.

**어려운 부분은 이미 쓰여 있었다.** `ingest_agg_trade`가 집계 체결 id로 중복을
제거하고, id가 건너뛰면 REST로 갭을 복구하고, 저장 전에 순서를 검증하고,
체크포인트를 전진시킨다. 끊긴 동안 놓친 것은 재연결 후 첫 프레임이 복구한다 —
id가 건너뛰어 있으므로.

**판단이 필요했던 것은 어떤 실패가 재연결하고 어떤 실패가 멈추는가다.** 연결이
끊기는 것은 평범하다(Binance가 매일 스스로 닫는다). correction conflict는 아니다 —
이미 저장한 체결에 대해 거래소가 다른 말을 한다는 뜻이고, 그걸 삼키는 스트림은
건강해 보이면서 우리 기록이 테이프에서 갈라지게 둔다. 그건 전파한다.

**테스트가 진짜 버그를 잡았다.** 깨끗하게 끝난 연결(예외 없이 iterator 소진)이
백오프 없이 즉시 재연결해서 핫 루프가 됐다. 서버가 정상 종료해도 그렇다. 이제
모든 종료가 백오프하고, 프레임을 만들어냈는지 여부가 얼마나 물러설지만 정한다.

`websockets`를 의존성으로 선언했다. `uvicorn[standard]`를 통해 딸려오고 있었는데,
전략이 order flow를 읽는 피드가 다른 패키지가 계속 그걸 원하는지에 달려 있으면
안 된다.

### 이 기계에서 확인된 것과 확인되지 않은 것

**클라이언트는 끝에서 끝까지 동작한다.** 실제 Binance aggTrade 프레임 56건을
받아 스토어에 넣었다(테스트 DB, spot 스트림).

**그런데 futures 스트림이 이 기계에서 데이터를 주지 않는다.**

| 대상 | 결과 |
|---|---|
| `wss://fstream.binance.com/ws/btcusdt@aggTrade` | 연결됨, 15초간 프레임 0 |
| `wss://fstream.binance.com/stream?streams=...` | 같음 |
| `wss://stream.binance.com:9443/...` (spot) | **정상** |
| `wss://ws.postman-echo.com/raw` (대조군) | **정상** |
| `https://fstream.binance.com/` | 404 (호스트 정상) |
| `https://fapi.binance.com` REST | 세션 내내 정상 |

웹소켓 자체도, 호스트 도달도, futures REST도 문제없다. `fstream`만 업그레이드를
받아들이고 아무것도 보내지 않는다. 코드로 우회할 문제가 아니다.

**spot으로 바꾸지 않는다.** spot BTCUSDT는 다른 시장이고, 전략이 읽는 order flow는
전략이 거래하는 시장의 것이어야 한다. spot 프레임은 클라이언트 검증에만 썼고,
프로덕션 스토어가 아니라 테스트 DB에 넣었다.
