...
| F6 | 포지션 관리 모듈 전체가 호출되지 않음 (청산 9종 도달 불가) | **높음** | 미해결 · [조사 완료](2026-09-02-exit-path-investigation.md) |
...
`operations/david_v6_position.py`의 `manage_v6_position`을 부르는 곳이 **테스트뿐이다.** 프로덕션 호출자가 없다. 따라서 청산 행동 9종이 전부 도달 불가다 — 브레이크이븐 이동, 가산, fib 66 익절, 대형 체결 회피 청산, Método 이탈 청산, 비상 청산까지.

실제로 존재하는 청산은 **진입 체결 시 생성되는 초기 손절 하나**뿐이고, 그 손절은 움직이지도 익절하지도 않는다. 세션 마감 `must_be_flat`도 **새 진입을 막을 뿐** 보유 포지션을 닫지 않는다.

F7·F3과 같은 형태이나 규모가 다르다. 저 둘은 입력 하나였고 이것은 **전략의 청산 측면 전체**다.

섀도는 포지션을 열지 않으므로 지금은 무해하다. **페이퍼 승격의 선행 조건**이다.
